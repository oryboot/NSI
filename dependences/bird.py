    ...
    def attaquer(self,ennemi,indice=0) :
        ...
        while not attaque_valide :
            ...
            if attaque=='1' :#si le joueur choisit de lancer une attaque normale
                if self.nom=='Bomb' :
                    if self.attaque<ennemi[indice].defense:#si la défense de l'ennemi est supérieure à  l'attaque du joueur
                        ennemi[indice].PV-=0#le joueur ne lui fait aucun dégà¢t
                        print(self.nom,'attaque',ennemi[indice].nom,': Vous ne faites aucun dégà¢ts.')#On le précise
                    else :
                        ...
                        print(self.nom,'attaque',ennemi[indice].nom,': Vous faites',self.attaque-ennemi[indice].defense,'dégà¢ts.')#On précise les dégà¢ts infligés
                else :
                    if self.attaque<ennemi.defense:#si la défense de l'ennemi est supérieure à  l'attaque du joueur
                        ...
                        print(self.nom,'attaque',ennemi.nom,': Vous faites',self.attaque-ennemi.defense,'dégà¢ts.')#On précise les dégà¢ts infligés
                    ...
                    if self.nom=='Chuck' :
                        ...
                        if self.att_spe[1]< ennemi.defense:#si la défense de l'ennemi est supérieure à  l'attaque du bird
                            ...
                            if ennemi.PV<0 :#si ses PV sont inférieurs à  0
                                ennemi.PV=0#on les passe à  0
                            print(self.nom,'attaque',ennemi.nom,': Vous faites',self.att_spe[1]-ennemi.defense,'dégà¢ts.')
                        ...
                        for enn in ennemi :
                            if self.att_spe[1]< enn.defense:#si la défense de l'ennemi est supérieure à  l'attaque du bird
                                ennPV-=0#le joueur ne fait aucun dégà¢t
